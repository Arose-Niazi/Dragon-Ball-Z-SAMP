...
¦ (command) spam, too.                    								¦
¦ If Sharingan kicks/bans too much innocent players, disable the codes ¦
¦ which kick/ban too many innocent players. You can easily configurate  ¦
¦ Sharingan in the file "Sharingan.cfg" in the folder "Sharingan"    ¦
...
¦                                                                       ¦
¦ The main script of Sharingan is no longer an include!     			¦
¦ It's a filterscript now! Use Sharingan.inc for all your        		¦
...
¦ - Remove this text                                                    ¦
¦ - Rename Sharingan! Never!                                 			¦
¦ - Re-release this script                                              ¦
¦ - Say it's your own script                                            ¦
¦ - Remove the "Sharingan:" tag from client messages!           		¦
¦                                                                       ¦
...
Changelog Update 11:
	...
	- It's now fully compatible with LuxAdmin (use #include <Sharingan_Slim> in LuxAdmin)
...
Changelog Update 10:
	...
	- If an admin accidently types the rcon-password into the global chat, Sharingan blocks it
	...
	- Made hooking easier in Sharingan.inc. Not important for you, just for me.
	...
	- Sharingan can now take action when a player has too many warnings in total
...
#define CONFIG_FILE 			"Sharingan/Sharingan.cfg"
#define JB_CHROME_FILE          "Sharingan/SharinganChrome.log"
#define BAD_WORDS_FILE 			"Sharingan/BadWords.cfg"
#define FORBIDDEN_WEAPONS_FILE 	"Sharingan/ForbiddenWeapons.cfg"
#define JB_LOG_FILE 			"Sharingan/Sharingan.log"
#define BLACKLIST_FILE 			"Sharingan/Blacklist.txt"
#define WHITELIST_FILE 			"Sharingan/Whitelist.txt"
#define IP_BAN_FILE 			"Sharingan/IpBans.txt"
#define TEMP_BAN_FILE 			"Sharingan/TempBans.txt"
#define BAD_RCON_LOGIN_FILE 	"Sharingan/BadRconLogin.txt"
...
{
	!"If enabled Sharingan will ban for forbidden weapons.\nSet to 0 to disable, set to 1 to enable.", //WeaponHack
	...
	!"Set max ping. Sharingan will calculate average ping for players and kick them if their ping is higher.\n Set to 0 to disable.\nIf you want to enable it, it's recommended to set higher than 300.", //MaxPing
	...
	!"Sharingan will constantly update the players freeze status.\nCheaters can unfreeze themselves but Sharingan will freeze them again if enabled.", //FreezeUpdate
	...
	!"Enable Sharingan Chrome to collect as much user data as possible\nwhen a player gets kicked. This may help you with ban appeals of users.\nSet to 1 to enable, set to 0 to disable.", // SharinganChrome
...
PRIVATE: JB::MutePlayer (playerid, time, reason [])//Time is seconds.
{
	if (IsPlayerConnected (playerid) && time)
	{
		...
		JB::SendFormattedMessageToAll (JB_RED, "Sharingan:{FFFF00} Muting player '%s' (%d) for %d second(s). Reason: %s", JB::PlayerInfo [playerid][JB::pName], playerid, time, reason);
		return 1;
	}
	return 0;
}
...
PRIVATE: JB::Kick (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		JB::SendFormattedMessageToAll (JB_RED, "Sharingan:{FFFF00} Kicking player '%s'. Reason: %s", JB::PlayerInfo [playerid][JB::pName], reason);
		JB::LogEx ("%s (%s) has been kicked for %s.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp], reason);
		CallRemoteFunction ("OnSharinganKick", "is", playerid, reason);
		Kick (playerid);
		return 1;
	}
	return 0;
...
PRIVATE: JB::Ban (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		TogglePlayerControllable (playerid, false);
		SetCameraBehindPlayer (playerid);
		JB::SendFormattedMessageToAll (JB_RED, "Eternal Mangekyou Sharingan:{FFFF00} Banning player '%s'. Reason: %s", JB::PlayerInfo [playerid][JB::pName], reason);
		JB::LogEx ("%s (%s) has been banned for %s.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp], reason);
		CallRemoteFunction ("OnSharinganBan", "is", playerid, reason);
...
PRIVATE: DisplayKickBanText (playerid, reason [], bool: ban)
{
	if (IsPlayerConnected (playerid))
	{
		...
		{
	        JB::KickBanTitle = TextDrawCreate (320.000000, 100.000000, ban ? ("Sharingan~n~Ban Info~n~~n~~n~~n~~n~~n~~n~~n~~n~") : ("Sharingan~n~Kick Info~n~~n~~n~~n~~n~~n~~n~~n~~n~"));
		    ...
		    TextDrawSetString (JB::KickBanTitle, ban ? ("Sharingan~n~Ban Info~n~~n~~n~~n~~n~~n~~n~~n~~n~") : ("Sharingan~n~Kick Info~n~~n~~n~~n~~n~~n~~n~~n~~n~"));
...
PUBLIC: SharinganChrome (playerid, comment [])
{
	if (IsPlayerConnected (playerid))
	{
		JB::LogEx ("Executing Sharingan Chrome for player '%s'.", JB::PlayerInfo [playerid][JB::pName]);
		new
			string [256],
			string2 [64],
			Float: fvar [9],
...
PUBLIC: CheckText (playerid, text []) //return 1: something forbidden found, return 0: everything OK!
{
	...
	{
		SendClientMessage (playerid, JB_RED, "Sharingan:{FFFF00} You are not allowed to use Caps Lock! Press [Caps Lock] to disable it.");
		return 1;
	}
 ...
 if (JB::Variables [SINGLEPLAYER_CHEATS] && SingleplayerCheatCheck (text))
	...
	{
		SendClientMessage (playerid, JB_RED, "Sharingan:{FFFF00} You are not allowed to use such words!");
		return 1;
	}
	return 0;
}
...
PRIVATE: SharinganReport (playerid, report [], details [])
{
	if (IsPlayerConnected (playerid))
	{
		new string [128];
		...
		format (string, sizeof (string), "Sharingan:{FFFF00} Suspected player with %s: %s (%d, %s), %s.", report, JB::PlayerInfo [playerid][JB::pName], playerid, JB::PlayerInfo [playerid][JB::pIp], details);
		ForEachPlayer (i)
			if (JB::IsPlayerAdmin (i) && i != playerid)
				SendClientMessage (i, JB_RED, string);

		CallRemoteFunction ("OnSharinganReport", "iss", playerid, report, details);
...
You can link your admin system with Sharingan,
		        ...
		        if (GetPlayerSurfingVehicleID (i) == INVALID_VEHICLE_ID && GetPlayerSurfingObjectID (i) == INVALID_OBJECT_ID && JB::GetPlayerSpeed (i, true) < 30 && (JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_DRIVER || JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_ONFOOT))
		        {
					...
					if (speed > JB::Variables [AIRBRAKE] && JB::PlayerInfo [i][JB::pLastAirbrakeSpeed] > JB::Variables [AIRBRAKE])
					{
						...
						if (JB::Warnings [i]{AIRBRAKE} < MAX_CHECKS)
						{
							...
							SharinganReport (i, "airbrake", reason);
						}
						else
						{
                        ...
                        if (!HasTimePassed (JB::PlayerInfo [i][JB::pLastUpdate], 2000))
                        {
                            ...
                            if (JB::Warnings [i]{MONEY_HACK} >= MAX_CHECKS)
                            {
								...
								else
								{
									...
									SharinganReport (i, "moneyhack", reason);
			...
			if (!JB::IsPlayerAdmin (i) && IsPlayerInValidState (i) && !DOB::GetBit (JB::AntiBugKilled, i)) //!JB::PlayerInfo [i][JB::pAntiBugKilled])
			{
			    ...
			    if (JB::Variables [JOYPAD] && !JB::PlayerInfo [i][JB::pFreezeTime])
				{
				    ...
				    if((updown != 128 && updown != 0 && updown != -128) || (leftright != 128 && leftright != 0 && leftright != -128))
				    {
						...
						else
						{
						    ...
						    SetSyncTime (i, SYNC_TYPE_VEHICLE);
							TogglePlayerControllable (i, false);
							SendClientMessage (i, JB_RED, "Sharingan:{FFFF00} Please stop using a joypad/gamepad. Use your keyboard and mouse.");
						}
				    }
				}
				...
				if (JB::Variables [MIN_FPS])
				{
					...
					if (var < JB::Variables [MIN_FPS])
					{
						...
						{
							JB::SendFormattedMessage (i, JB_RED, "Sharingan:{FFFF00} Please fix your framerate (FPS) or you will get kicked! (Min %d, your FPS: %d)", JB::Variables [MIN_FPS], var);
							SendClientMessage (i, JB_RED, "Sharingan:{FFFF00} Pressing F7 once may help to fix it. (Removes the outlines of the letters in chat window.)");
						}
					}
				}
							...
							if (!IsPlayerInRangeOfPoint (i, 5.0, JB::PlayerInfo [i][JB::pSetPos][0], JB::PlayerInfo [i][JB::pSetPos][1], JB::PlayerInfo [i][JB::pSetPos][2]))
							{
								...
								if (JB::Variables [TELEPORT_HACK])
								{
									...
									else if (JB::Warnings [i]{TELEPORT_HACK} >= MAX_CHECKS)
									{
										...
										SharinganReport (i, "teleport cheats", reason);
								}
								else
									SharinganReport (i, "teleport cheats", reason);
							}
								...
								if (!JB::PlayerInfo [i][JB::pFreezeTime])
								{
									if (var > JB::Variables [MAX_SPEED])
									{
										...
										if (JB::Warnings [i]{MAX_SPEED} < MAX_CHECKS)
										{
											...
											SharinganReport (i, "speedhack [Code 1]", reason);
										}
										else
										{
									...
									else if (JB::Variables [SPEEDHACK_ADVANCED] && var > (ivar [0] + JB::Variables [SPEEDHACK_ADVANCED]))
									{
										...
										{
											format (reason, sizeof (reason), "%d KM/H with %s (Max %d)", var, JB::GetVehicleName (vehicleid), ivar [0]);
											SharinganReport (i, "speedhack [Code 2]", reason);
										}
										else
										{
									...
									if (floatround (rz * 160.0) < JB::Variables [WALLRIDE] && var > 10)
									{
									    ...
									    if (JB::PlayerInfo [i][JB::pWallriding] == MAX_CHECKS)
									    {
										    ...
										    else
										    {
												...
												SharinganReport (i, "wallride", reason);
										    }
									    }
									}
							    }
		...
		for (new i = 0; i < TempBanCount; ++i)
		{
			if (!strcmp (TempBanInfo [i][tbName], JB::PlayerInfo [playerid][JB::pName], false))
			{
				if (TempBanInfo [i][tbTime] > time)
				{
					...
					JB::SendFormattedMessage (playerid, JB_RED, "Sharingan:{FFFF00} You are temporary for %d day(s), %d hour(s), %d minute(s) and %d second(s)!", days, hours, minutes, seconds);
...
PRIVATE: ConfigSharingan ()
{
	...
	print (" ");
	JB::Log ("Current Sharingan configuration: ");
	for (new i = 0; i < MAX_JB_VARIABLES; ++i)
	...
	JB::Log ("Sharingan has been configurated.");
	return 1;
}
...
PUBLIC: SaveSharinganVars ()
{
	...
	print (" ");
	JB::Log ("Sharingan configuration has been saved to file.");
	return 1;
}
...
{
	ShowPlayerDialog (playerid, DIALOG_CFG, DIALOG_STYLE_LIST, "Sharingan", "Set a variable\nLoad configuration from file\nSave configuration to file\nLoad default configuration\nSee Sharingan reports\nSee Sharingan commands for admins", "Choose", "Close");
}
...
PRIVATE: ShowPlayerHackCodeDialog (playerid)
{
	...
	strcat (string, "{0CB7EB}Health-/armourhack:{FFFFFF}\n\tCode 1: Health/armour is higher than 100.\n\tCode 2: Player's health/armour is higher than Sharingan actually expects.");
	ShowPlayerDialog (playerid, DIALOG_HACKCODES, DIALOG_STYLE_MSGBOX, "Sharingan hack codes", string, "Close", "");
}
...
PRIVATE: ShowPlayerVarlistDialog (playerid)
{
	...
	ShowPlayerDialog (playerid, DIALOG_VARLIST, DIALOG_STYLE_LIST, "Sharingan variables", string, "Choose", "Go back");
}
...
PRIVATE: ShowPlayerSetvarDialog (playerid, var)
{
	...
	format (string, sizeof (string), "Sharingan variable: %s = %d", JB::VariableNames [var], JB::Variables [var]);
...
PRIVATE: ShowPlayerReportDialog (playerid)
{
	...
	if (string [0])
		ShowPlayerDialog (playerid, DIALOG_REPORTS, DIALOG_STYLE_LIST, "Sharingan reports", string, "Close", "");
	else
	    ShowPlayerDialog (playerid, DIALOG_REPORTS, DIALOG_STYLE_MSGBOX, "Sharingan reports", "There are no reports.", "Close", "");
}
...
CALLBACK: OnPlayerGodMode (playerid)
{
	...
	SharinganReport (playerid, "godmode", "no details");
	return 1;
}
	    ...
	    new Float: x, Float: y, Float: z;
 		...
 		if (GetVehicleDistanceFromPoint (vehicleid, x, y, z) < 5.0)
 		{
 		    if (++JB::Warnings [playerid]{SPAWN_VEHICLES} >= JB::Variables [SPAWN_VEHICLES])
 		    {
 		        ...
 		        SharinganReport (playerid, "spawning vehicle", JB::GetVehicleName (vehicleid));
 		}
	}
	        ...
	        if (JB::PlayerInfo [playerid][JB::pLastVehicle] != vehicleid)
	        {
			    if (!JB::SyncInfo [playerid][SYNC_TYPE_POS][JB::sSyncTime])
			    {
					if (JB::Variables [VEHICLE_TELEPORT])
					{
			            ...
			            SharinganReport (playerid, "vehicle teleport", JB::GetVehicleName (vehicleid));
			    }
		    }
		    else if (JB::Variables [VEHICLE_REPAIR] && !JB::SyncInfo [playerid][SYNC_TYPE_VEHICLE][JB::sSyncTime])
		    {
		        if (fvar > JB::PlayerInfo [playerid][JB::pVehicleHealth])
		        {
		            ...
		            if (JB::Warnings [playerid]{VEHICLE_REPAIR} >= JB::Variables [VEHICLE_REPAIR])
		            {
		                ...
		                SharinganReport (playerid, "repairing vehicle with cheats", JB::GetVehicleName (vehicleid));
		        }
		    }
	    }
		...
		if (JB::Variables [SPEEDHACK_DETECTION] != 0 && JB::PlayerInfo [playerid][JB::pCurrentState] == PLAYER_STATE_DRIVER && !JB::PlayerInfo [playerid][JB::pFreezeTime])
		{
			...
			if (speed > JB::Variables [MAX_SPEED])
			{
				...
				if (JB::Warnings [playerid]{MAX_SPEED} < MAX_CHECKS)
				{
					...
					SharinganReport (playerid, "speedhack [Code 1]", reason);
				}
				else
				{
			...
			else if (JB::Variables [SPEEDHACK_ADVANCED] && speed > (maxspeed + JB::Variables [SPEEDHACK_ADVANCED]))
			{
				...
				{
					format (reason, sizeof (reason), "%d KM/H with %s (Max %d)", speed, JB::GetVehicleName (vehicleid), maxspeed);
					SharinganReport (playerid, "speedhack [Code 2]", reason);
				}
				else
				{
				...
				else
				{
					...
					if (JB::PlayerInfo [playerid][JB::pAmmoUsed][ivar] > (AmmoAmount [ivar] * 2) && ammoused > 0)//Player must have switch weapons fast or doesn't reload at all
					{
						...
						if ((JB::Variables [NO_RELOAD] == 1 && (JB::Warnings [playerid]{NO_RELOAD} % 10) == 0))
						{
							...
							SharinganReport (playerid, "no reload", reason);
						}
		    ...
		    if (JB::PlayerInfo [killerid][JB::pCurrentState] == PLAYER_STATE_DRIVER)
		    {
				if (JB::Variables [DRIVE_BY] >= 2 && (reason == WEAPON_UZI || reason == WEAPON_MP5 || reason == WEAPON_TEC9 || reason == 50)) // 50 = helicopter blades.
				{
					...
					else
					{
						...
						SendClientMessage (killerid, JB_RED, "Sharingan:{FFFF00} You have been killed for drive-by!");
					}
				}
			 	...
			 	if (JB::Variables [ARMED_VEHICLES])
			 	{
					...
					if (IsArmedVehicle (vehicleid))
					{
					    if (++JB::Warnings [killerid]{ARMED_VEHICLES} >= JB::Variables [ARMED_VEHICLES])
						{
							...
							JB::SendFormattedMessage (killerid, JB_RED, "Sharingan:{FFFF00} Killing with a %s is not allowed! Stop it! (Warning %d/%d)", JB::GetVehicleName (vehicleid), JB::Warnings [killerid]{ARMED_VEHICLES}, JB::Variables [ARMED_VEHICLES]);
					}
			 	}
			}
			...
			if (JB::Variables [SPAWNKILL])
			{
				if (JB::PlayerInfo [playerid][JB::pSpawnKillProtected])
				{
					if (++JB::Warnings [killerid]{SPAWNKILL} >= JB::Variables [SPAWNKILL])
						...
						JB::SendFormattedMessage (killerid, JB_RED, "Sharingan:{FFFF00} Do not spawnkill! (Warning %d/%d)", JB::Warnings [killerid]{SPAWNKILL}, JB::Variables [SPAWNKILL]);
				}
		 	}
		}
...
CALLBACK: OnPlayerConnect (playerid)
{
	...
	if (!IsPlayerNPC (playerid))
	{
	        ...
	        if (JB::Variables [BLACKLIST])
	        {
		        ...
		        if (banned)
		        {
		            ...
		            {
		                SendClientMessage (playerid, JB_RED, "Sharingan:{FFFF00} You are banned from this server!");
						JB::LogEx ("%s (%s) has been banned for Ban evading.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp]);
					...
					else if (t < unbantime)
					{
					    ...
					    JB::SendFormattedMessageToAll (JB_RED, "Sharingan:{FFFF00} You are temporary for %d day(s), %d hour(s), %d minute(s) and %d second(s)!", days, hours, minutes, seconds);
	...
	{
		SendClientMessage (playerid, JB_GREEN_BLUE, "> This server is running {FF0000}Sharingan Anti-Cheat "#JB_VERSION"{00D799}!");
...
CALLBACK: OnPlayerDisconnect (playerid, reason)
{
	...
	if (reason == 2 && JB::Variables [JB_CHROME])
	    SharinganChrome (playerid, "No comment");
...
CALLBACK: OnPlayerText (playerid, text [])
{
	...
	{
		JB::SendFormattedMessage (playerid, JB_RED, "Sharingan:{FFFF00} You are not allowed to chat for %d second(s) because you have been muted!", JB::PlayerInfo [playerid][JB::pMuted]);
		return 0;
	}
...
//==============================================================================
	...
	COMMAND:jbtest (playerid, params [])
	{
		...
		t = GetTickCount () - t;
		JB::SendFormattedMessage (playerid, JB_RED, "Sharingan time: %d ms", t);
		return 1;
	}
...
COMMAND:jbcmds (playerid, params [])
{
	...
	ShowPlayerDialog (playerid, DIALOG_CMDS, DIALOG_STYLE_LIST, "Sharingan Commands", string, "Perform", "Close");
	return 1;
}
...
COMMAND:jbchrome (playerid, params [])
{
	...
	if (SharinganChrome (id, JB::PlayerInfo [playerid][JB::pName]))
		JB::SendFormattedMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Sharingan Chrome has successfully collected data of player '%s'!", JB::PlayerInfo  [id][JB::pName]);
	else
	    JB::SendFormattedMessage (playerid, JB_RED, "Sharingan:{FFFF00} Sharingan Chrome could not colect data of player '%s'!", params);
	return 1;
}
...
COMMAND:jbsethomepage (playerid, params [])
{
    ...
    DOF2::SetString (CONFIG_FILE, "Homepage", JB::Homepage, "Misc");
    JB::SendFormattedMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Homepage has been set to '%s'!", params);
	return 1;
}
...
COMMAND:blackadd (playerid, params [])
{
	...
	if (!IsPlayerConnected (id))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Player '%s' has successfully been added to blacklist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "Sharingan:{FFFF00} Could not add player '%s' to blacklist!", params);
	}
	else
	{
		if (id != playerid)
		{
		        ...
		        {
		            JB::SendFormattedMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Player '%s' has successfully been added to blacklist!", JB::PlayerInfo [id][JB::pName]);
				...
				if (AddPlayerToBlacklist (id))
					...
					JB::SendFormattedMessage (playerid, JB_RED, "Sharingan:{FFFF00} Could not add player '%s' to blacklist!", JB::PlayerInfo [id][JB::pName]);
		}
	}
	return 1;
}
...
COMMAND:blackdel (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Player '%s' has successfully been removed from blacklist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "Sharingan:{FFFF00} Could not remove player '%s' from blacklist!", params);
			return 1;
	}
...
COMMAND:whiteadd (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Player '%s' has successfully been added to whitelist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "Sharingan:{FFFF00} Could not add player '%s' to whitelist!", params);
			return 1;
	}
...
COMMAND:whitedel (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Player '%s' has successfully been removed from whitelist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "Sharingan:{FFFF00} Could not remove player '%s' from whitelist!", params);
			return 1;
	}
...
COMMAND:jbsetvar (playerid, params [])
{
	...
	{
	    JB::SendFormattedMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Sharingan variable '%s' has successfully been set to %d.", var, value);
...
COMMAND:tunban (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Temporary ban of player '%s' has successfully been deleted.", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "Sharingan:{FFFF00} Could not delete temporary ban of player '%s'!", params);
			return 1;
	}
...
COMMAND:banip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
	    ...
	    SendClientMessage (playerid, JB_RED, "Sharingan:{FFFF00} Sorry, this command is currently not available when using Sharingan database.");
	    return 1;
	#else
		if (!isnull (params))
		{
			...
			{
				if (BanIp (params))
					JB::SendFormattedMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} IP %s has successfully been banned!", params);
				else
					JB::SendFormattedMessage (playerid, JB_RED, "Sharingan:{FFFF00} Could not ban IP %s!", params);
				return 1;
			}
		}
...
COMMAND:unbanip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
	    ...
	    if (JBDB::UnbanIp (params))
			JB::SendFormattedMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} IP %s has successfully been unbanned!", params);
		else
			JB::SendFormattedMessage (playerid, JB_RED, "Sharingan:{FFFF00} Could not unban IP %s!", params);
		return 1;
	#else
		if (!isnull (params))
		{
		...
		if (JB::PlayerInfo [playerid][JB::pVehicleEntered] != vehicleid && !JB::IsPlayerAdmin (playerid) && !IsPlayerNPC (playerid))
		{
			...
			if (driver != INVALID_PLAYER_ID)
			{
				...
				if (JB::Variables [CAR_JACK_HACK] && JB::Warnings [playerid]{CAR_JACK_HACK} >= MAX_CHECKS)
					...
					SharinganReport (playerid, "carjack hack", JB::PlayerInfo [driver][JB::pName]);
			}
				...
				case 24, 25, 27, 29, 30, 31, 33, 34: // Deagle, Shotung, SPAS12, MP5, AK47, M4, Rifle, Sniper Rifle
				{
					...
					else if (((oldkeys & KEY_FIRE) || (oldkeys == KEY_FIRE)) && ((newkeys & KEY_CROUCH) || (newkeys == KEY_CROUCH)) && (GetTickCount () - JB::PlayerInfo [playerid][JB::pFired]) < 750)
					{
						if (++JB::Warnings [playerid]{CBUG} & 1 == 0)
							SendClientMessage (playerid, JB_RED, "Sharingan:{FFFF00} Please stop performing the C-Bug or you will get kicked/banned.");
	...
	if (1 <= JB::Variables [DRIVE_BY] <= 2 && PRESSED (KEY_FIRE) && (HOLDING (KEY_LOOK_RIGHT) || HOLDING (KEY_LOOK_LEFT)))
	{
		if (!IsPlayerInPlane (playerid) && GetVehicleModel (GetPlayerVehicleID (playerid)) != 432)//Ignore planes and rhinos.
		{
			...
			if (weaponid != 0 && ammo != 0)//Check if player has got an SMG. Don't punish innoncent players.
			{
				...
				SendClientMessage (playerid, JB_RED, "Sharingan:{FFFF00} Please stop performing drive-by!");
				TogglePlayerControllable (playerid, false);
			...
			case DIALOG_CFG:
			{
				if (response)
				{
					switch (listitem)
					{
						...
						{
							ConfigSharingan ();
							SendClientMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Configuration has been loaded from file.");
							ShowPlayerConfigDialog (playerid);
						}
						...
						{
							SaveSharinganVars ();
							SendClientMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Configuration has been saved to file.");
							ShowPlayerConfigDialog (playerid);
						}
						...
						{
							JB::Variables = JB::DefaultVariables;
							SendClientMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Default configuration has been loaded.");
	    					JB::Log ("Sharingan variables have been reset to default values.");
							ShowPlayerConfigDialog (playerid);
						}
			...
			case DIALOG_SETVAR .. (DIALOG_SETVAR + MAX_JB_VARIABLES - 1):
			{
				if (response)
				{
					...
					else
					{
						...
						JB::SendFormattedMessage (playerid, JB_GREEN, "Sharingan:{FFFF00} Sharingan variable '%s' has successfully been set to %d.", JB::VariableNames [var], JB::Variables [var]);
...
CALLBACK: OnRconCommand (cmd [])
{
	...
	{
	    JB::Variables = JB::DefaultVariables;
	    JB::Log ("Sharingan variables have been reset to default values by an RCON admin.");
	    return 1;
	}
		...
		if (JB::Variables [CHECKPOINT_TELEPORT])
		{
			if (!JB::GetPlayerSpeed (playerid, false))
			{
				...
				else
				{
					...
					SharinganReport (playerid, "teleport cheats", string);
				}
			}
		}
	}
